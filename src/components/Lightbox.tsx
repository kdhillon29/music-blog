...
function LightBox({setOpen,images,thumbnails,index,setIndex,currntImage,setCurrntImage}: Props){

    let isPageWide = UseMediaQuery('(max-width: 900px)')
    let imageHolder =useRef<HTMLDivElement>(null)
    let thumbnailHolder = useRef<HTMLElement[]>([])
    thumbnailHolder.current = [];
    const addRef = (el:HTMLLIElement)=>{
    ...
    function ViewrUpdate(){
        ...
        const viewr = imageHolder.current?.lastElementChild as HTMLImageElement
    ...
    return(
      <div class="absolute space-y-7 flex flex-col justify-center items-center top-0 bottom-0 left-0 right-0">
           <div class="fixed z-10 bg-black clear-both opacity-50 w-full h-full top-0 bottom-0" onClick={()=>setOpen(false)}></div>
            <div class="relative z-20 w-1/3" ref={el=> imageHolder.current=el}>
